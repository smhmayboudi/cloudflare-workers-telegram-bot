...
export default class TelegramApi extends BotApi {
  ...
  greetUsers = async (update: Update): Promise<Response> => {
    ...
    return isMessageUpdate(update) &&
      isNewChatMembersMessage(update) &&
      isTelegramPublicChat(update.message.chat)
      ? this.sendMessage(
  ...
  executeInlineCommand = async (update: Update): Promise<Response> => {
    ...
    return this._executeCommand(
      update,
      isInlineQueryUpdate(update) ? update.inline_query.query : "",
    ).then(async (command_response) =>
      command_response
  ...
  executeCommand = async (update: Update): Promise<Response> => {
    ...
    return (
      this._executeCommand(
        update,
        isMessageUpdate(update) && isTextMessage(update.message)
          ? update.message.text
          : "",
      ) ?? this.updates.default()
    );
